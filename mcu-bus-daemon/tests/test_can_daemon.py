...
from domain.mcu_bus import AlertEvent, HeartbeatEvent, TelemetryEvent
...
def test_handle_temp_hum_publishes_sensor_event():
    ...
    assert isinstance(event.payload, TelemetryEvent)
    assert event.source_node_id == 1
    assert event.payload.readings[0].value == 25.34
    assert event.payload.readings[1].value == 60.27
...
def test_handle_heartbeat_updates_last_seen_without_publishing_ok_status():
    ...
    assert isinstance(event.payload, HeartbeatEvent)
